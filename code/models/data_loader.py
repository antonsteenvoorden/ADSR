...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Randomly select one target genre
        nonzeroes = torch.nonzero(context_y).view(-1, 1)
        if len(nonzeroes) == 0:
            context_y = 0
        elif len(nonzeroes) == 1:
            context_y = nonzeroes.item()
        else:
            context_y = torch.randint(low=0, high=len(nonzeroes), size=(1,)).item()
