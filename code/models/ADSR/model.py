...
class ADSR(nn.Module):
    def __init__(self, item_num, context_num, user_num,
        ...
        self.context_encoder_GRU = nn.GRU(input_size=num_context_units + num_item_units,
                                          hidden_size=num_gru_units,
                                          batch_first=True,
                                          bidirectional=True)
    ...
    def forward(self, inputs):
        ...
        compare_tensor = torch.tensor([150, 0, 0, 0, 0, 0, 0, 0, 0], device=self.device)
        for index, x in enumerate(item_x):
            if x.eq(compare_tensor).sum() >= len(compare_tensor):
    ...
    def encode(self, item_x, context_x, user_id):
        ...
        context_input = torch.cat([item_x_embedded, context_x_embedded], dim=-1)
