...
class MPMCRN_Manager(Model):
    ...
    def __init__(self, args):
        ...
        super(MPMCRN_Manager, self).__init__(required_keys, **vars(args))
        ...
        _loss_function = get_loss_fn(item_num, BCE=self.BCE)
    ...
    def _set_evaluator(self, model, loss_fn, embeddings_path):
        evaluator = create_supervised_evaluator(model, device=self.device,
                                                metrics={
                                                    ...
                                                    'discrete': DiscreteDiversity(embeddings_path, self.item_num,
                                                                                  self.metrics_save_path)
                                                }, non_blocking=False)
        self.evaluator = evaluator

    def _set_dataloaders(self, train_path, valid_path, test_path, batch_size, num_workers=4):
        train_loader, val_loader, test_loader, item_num, user_num = get_data_loaders(train_path,
                                                                                     valid_path,
                                                                                     test_path,
                                                                                     batch_size,
