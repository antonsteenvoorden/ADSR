...
class RetrievalMetrics(Metric):
    ...
    def __init__(self, save_path):
        ...
        self._context_predictions = None
        self._context_probs = None
    ...
    def update(self, output):
        ...
        if context_logits is not None:
            item_y, context_y = split_input(y, dtype=torch.int, requires_grad=False, device=device)
        else:
        ...
        if context_predictions is not None:
            ...
            self._statistics[CONTEXT_MIN].extend(context_probs.min(dim=-1).values)
            self._statistics[CONTEXT_MAX].extend(context_probs.max(dim=-1).values)
    ...
    def compute(self):
        ...
        if len(self._attentions) > 0:
            with open(self.save_path + "_attention.txt", 'w') as f:
                for attn in self._attentions:
                    f.write(",".join([str(a.item()) for a in attn]))
                    f.write("\n")
            f.close()

        if len(self._context_probs) > 0:
            with open(self.save_path + "_contextprobs.txt", 'w') as f:
                for ctxt_prob in self._context_probs:
                    f.write(",".join([str(a.item()) for a in ctxt_prob]))
                    f.write("\n")
            f.close()
        ...
        # go over all context input samples, per context type
        for index, prediction in enumerate(self._context_predictions):
            label = self._context_labels[index]
            if isinstance(label, int):
                correct += int(prediction == label)
            else:
                correct += int(label[prediction] == 1)

        return {'recall': recall / count, 'mrr': mrr / count, "context_accuracy": correct / count}
...
class ContextAccuracy(RetrievalMetrics):
    def compute(self):
        retrieval_metrics = super(ContextAccuracy, self).compute()
        return retrieval_metrics[f'context_accuracy']
...
class CustomLoss(Metric):
    ...
    def update(self, output):
        ...
        combined_loss, relevance_loss, context_loss, diversity_loss = None, None, None, None
        if isinstance(losses, tuple):
            if len(losses) == 2:
                (combined_loss, relevance_loss) = losses
            elif len(losses) == 3:
                (combined_loss, relevance_loss, context_loss) = losses
            elif len(losses) == 4:
                (combined_loss, relevance_loss, context_loss, diversity_loss) = losses
        ...
        if diversity_loss is not None:
            self._sum['diversity_loss'] += diversity_loss.item() * N
            self._num_examples['diversity_loss'] += N

        if context_loss is not None:
            self._sum['context_loss'] += context_loss.item() * N
            self._num_examples['context_loss'] += N
    ...
    def compute(self):
        ...
        if "diversity_loss" in self._sum:
            diversity_loss = self._sum['diversity_loss'] / self._num_examples['diversity_loss']
            results["diversity_loss"] = diversity_loss

        if "context_loss" in self._sum:
            context_loss = self._sum['context_loss'] / self._num_examples['context_loss']
...
class DiversityLoss(CustomLoss):
    def compute(self):
        retrieval_metrics = super(DiversityLoss, self).compute()
        return retrieval_metrics.get('diversity_loss', 0)


class ContextLoss(CustomLoss):
    def compute(self):
        retrieval_metrics = super(ContextLoss, self).compute()
        return retrieval_metrics.get('context_loss', 0)
