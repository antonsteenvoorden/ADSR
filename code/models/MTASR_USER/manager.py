...
from .model import MTASR_USER
from .losses import get_loss_function_with_parameters
# CUSTOM
from .data_loader import get_data_loaders
from ..metrics import MRR, Recall, CombinedLoss, RelevanceLoss, ContextLoss, ContextAccuracy, DiscreteDiversity, ILD


class MTASR_USER_Manager(Model):
    name = "Multi-Task Attribute-aware Sequential Recommender extends ANAM with the AP module. " \
           "It predicts the attribute preference using the representations for the item sequence and " \
           "the item attribute sequence AND the user identifier. " \
           "This model uses the distribution over the attributes to form a weighted attribute embedding, " \
           "which is then combined with the attentive hidden representations to make predictions."
    ...
    def __init__(self, args):
        ...
        super(MTASR_USER_Manager, self).__init__(required_keys, **vars(args))
        ...
        _loss_function = get_loss_function_with_parameters(self.lambda_multitask_loss)
    ...
    def _set_trainer(self, model, optimizer, loss_fn,
        ...
        def _update(engine, batch):
            ...
            y_pred = model(x)
            (combined_loss, relevance_loss, context_loss) = loss_fn(y_pred, y)
